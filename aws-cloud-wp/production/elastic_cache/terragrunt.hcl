...
dependencies {
  paths = ["../wp_vpc", "../redis_security", "../aws-data"]
}
...
inputs = {
  ...
  subnet_ids = dependency.wp_vpc.outputs.public_subnets
  vpc_id = dependency.wp_vpc.outputs.vpc_id

  availability_zones = dependency.aws-data.outputs.available_aws_availability_zones_names
  namespace = "WP"
  stage = "cloud"

  subnets = dependency.wp_vpc.outputs.public_subnets
  #cluster_size = 1
  #family = "redis6.x"
  #engine_version = "6.x"
